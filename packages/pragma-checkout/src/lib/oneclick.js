  ...
  export const getOneClickCheckoutUrl = async (checkoutObject) => {
    ...
    const user = getUser();
    let encodedUser = encode(JSON.stringify(user))
    let encodedResponse = '';
    ...
    const url = `https://pay.1checkout.ai/v4/checkout/widget/?showerror=false&checkout_type=checkout&containerheight=720&apikey=${API_KEY_1C}&response=${encodedResponse}&user=${encodedUser}`;
    return url;
  };
